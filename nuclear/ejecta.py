...
class Ejecta(object):
    ...
    def from_yann_file(cls, fname):
        data = pd.read_table(fname, names=["isotope", "mass"], delim_whitespace=True)
    ...
    def __init__(self, mass_msol, composition):
        ...
        self.inventory = Inventory(self._normalize_composition(composition), "g")
        self._pad_material()
        atomic_masses = self.get_masses()
        self.n_per_g = [
            1 / atomic_masses[item] for item in self.get_all_children_nuc_name()
        ]
    ...
    def __getitem__(self, item):
        return self.inventory.__getitem__(item)

    def __setitem__(self, key, value):
        self.inventory.__setitem__(key, value)

    def keys(self):
        return self.inventory.contents.keys()

    @property
    def isotopes(self):
        return [Z_to_elem(id) for id in self.keys()]

    def get_decay_constant(self):
        return OrderedDict(
            (nuc_name, DEFAULTDATA.scipy_data.decay_consts(nuc_id))
            for nuc_id, nuc_name in zip(
                self.get_all_children(), self.get_all_children_nuc_name()
            )
        )

    def get_half_life(self):
        return [DEFAULTDATA.half_life(nuc_id) for nuc_id in self.keys()]

    def get_masses(self):
        return {
            nuc_name: DEFAULTDATA.scipy_data.atomic_masses(nuc_id) * u_to_g
    ...
    def get_all_children(self):
        ...
        def get_child(nuc_id):
            children = DEFAULTDATA.progeny(nuc_id)
            if len(children) != 0:
    ...
    def get_all_children_nuc_name(self):
        return [Z_to_elem(nuc_id) for nuc_id in self.get_all_children()]

    @staticmethod
    def _normalize_composition(composition):
        composition_sum = np.sum(list(composition.values()))
        normed_composition = {
            key: value / composition_sum for key, value in composition.items()
        }
    ...
    def _pad_material(self):
        for isotope in self.get_all_children_nuc_name():
            try:
                self.inventory[isotope]
            except KeyError:
                self.inventory[isotope] = 0.0
    ...
    def decay(self, epochs):
        ...
        decayed_fractions = pd.DataFrame(
            index=epochs.value, columns=columns, dtype=np.float64
        )
        day_to_s = 24 * 3600
        for epoch, row in decayed_fractions.iterrows():
            new_material = self.inventory.decay(epoch * day_to_s)
            fractions = [
    ...
    def get_numbers(self):
        N = {}
        for nuc_id, nuc_name in zip(self.keys(), self.isotopes):
            mass = self.inventory[nuc_id] * self.mass_g
            N[nuc_name] = (
                1 / (DEFAULTDATA.scipy_data.atomic_masses(nuc_id) * u_to_g) * mass
            )
        return N
